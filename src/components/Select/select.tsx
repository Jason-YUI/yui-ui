...
const addPrefixNode = (nodes: any, prefixIcon: React.ReactNode) => {
  ...
  addNode.setAttribute('class', 'yui-select-prefix-wrapper')

  // dom container insert
  const currentDom = nodes.current
  currentDom &&
    currentDom
      .querySelector('.yui-select')
      .insertBefore(addNode, currentDom.querySelector('.ant-select-selector'))

  // icon insert
  ReactDOM.render(
    prefixIcon as any,
    currentDom.querySelector('.yui-select-prefix-wrapper'),
  )
}
...
export const Select = forwardRef<SelectRef, FRCSelectProps>((props, ref) => {
  ...
  const classes = classNames('yui-select', className, {
    [`yui-select-prefix`]: prefixIcon,
    [`yui-select-prefix-icon-disabled`]: disabled,
  })

  const classesDropdown = classNames('yui-select', dropdownClassName, {
    [`yui-select-prefix-dropdown`]: prefixIcon,
    [`yui-select-remove-selected-icon`]: removeMenuItemSelectedIcon,
  })
  ...
  return (
    <div ref={nodes} className="yui-select-container">
